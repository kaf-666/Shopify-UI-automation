"""Cart Drawer Quantity Cases 入口。

用法：
    python scripts/run_cart_quantity_cases.py                   # both
    python scripts/run_cart_quantity_cases.py --viewport desktop
    python scripts/run_cart_quantity_cases.py --viewport mobile

产物：
    artifacts/cart-quantity/<run_id>/results.json
    artifacts/cart-quantity/<run_id>/<viewport>/<CASE_ID>-failure.png
...
from tests.cart_quantity_cases import CartQuantityCaseRunner
...
ARTIFACT_ROOT = PROJECT_ROOT / "artifacts" / "cart-quantity"


def run_viewport(viewport: str, artifact_dir: Path) -> Tuple[List, CartQuantityCaseRunner, dict]:
    ...
    try:
        ...
        runner = CartQuantityCaseRunner(runtime, site, viewport, artifact_dir=artifact_dir)
...
def print_viewport(viewport: str, results: List, runner: Optional[CartQuantityCaseRunner] = None) -> None:
    ...
    if runner is not None:
        print(f"{'':<14} Pre-clean: {runner.pre_clean_status} | Cleanup: {runner.cleanup_status}")
    print()
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Mondressy Cart Drawer Quantity Cases")
    ...
    print("=== Mondressy Cart Drawer Quantity Cases ===")
    print()
    ...
    print()
    print("Results:")
    print(f"artifacts/cart-quantity/{run_id}/results.json")
