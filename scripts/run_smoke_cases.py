"""正式 Smoke Cases 入口（结构化 JSON 结果）。

用法：
    python scripts/run_smoke_cases.py                  # both
    python scripts/run_smoke_cases.py --viewport desktop
    python scripts/run_smoke_cases.py --viewport mobile

产物：
    artifacts/smoke/<run_id>/results.json
    artifacts/smoke/<run_id>/<viewport>/<CASE_ID>-failure.png   （仅 FAIL）

退出码：0 = 全部通过，1 = 任一 FAIL/BLOCKED 或结果写入失败，2 = 非法视口。
"""
...
def residual_cart_items(runtime) -> int:
...
def run_viewport(viewport: str, artifact_dir: Path) -> Tuple[List, SmokeCaseRunner, int, dict]:
    ...
    try:
        ...
        return results, runner, residual_after, runtime_meta
    finally:
        close_browser(runtime)


def print_viewport(viewport: str, results: List, runner: SmokeCaseRunner, residual: int) -> None:
    ...
    print(
        f"{'':<14} Pre-clean: {runner.pre_clean_status}"
        + (f" ({runner.pre_clean_error})" if runner.pre_clean_error else "")
        + f" | Final cleanup: {runner.cleanup_status} | Residual cart: {residual}"
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Mondressy Smoke Cases")
    ...
    print("=== Mondressy UI Smoke Cases ===")
    print()
    vp_results: List[ViewportResult] = []
    for vp in viewports:
        vp_started_ts = iso_now()
        vp_started = time.perf_counter()
        results, runner, residual_after, runtime_meta = run_viewport(vp, artifact_dir)
        ...
        vp_status = (
            "PASS"
            if cases_ok
            and runner.pre_clean_status == "PASS"
            and runner.cleanup_status == "PASS"
            and residual_after == 0
            else "FAIL"
        )
        vp_results.append(
            ViewportResult(
                ...
                pre_clean={"status": runner.pre_clean_status, "residual_items_before": runner.residual_items_before},
                cleanup={"status": runner.cleanup_status, "residual_items_after": residual_after},
                cases=results,
            )
        )
        print_viewport(vp, results, runner, residual_after)
    ...
    print()
    print("Results:")
    print(f"artifacts/smoke/{run_id}/results.json")
