...
用法：
    python scripts/run_expanded_plp_cases.py                  # both
    python scripts/run_expanded_plp_cases.py --viewport desktop
    python scripts/run_expanded_plp_cases.py --viewport mobile

产物：
    artifacts/expanded-plp/<run_id>/results.json
    artifacts/expanded-plp/<run_id>/<viewport>/<CASE_ID>-failure.png
...
from tests.expanded_plp_cases import PlpCaseRunner
...
ARTIFACT_ROOT = PROJECT_ROOT / "artifacts" / "expanded-plp"


def run_viewport(viewport: str, artifact_dir: Path) -> Tuple[List, PlpCaseRunner, dict]:
    ...
    try:
        ...
        runner = PlpCaseRunner(runtime, site, viewport, artifact_dir=artifact_dir)
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Mondressy 扩展 PLP Cases")
    ...
    print("=== Mondressy Expanded PLP Cases ===")
    print()
    ...
    print()
    print("Results:")
    print(f"artifacts/expanded-plp/{run_id}/results.json")
