...
用法：
    python scripts/run_search_cases.py                   # both
    python scripts/run_search_cases.py --viewport desktop
    python scripts/run_search_cases.py --viewport mobile

产物：
    artifacts/search/<run_id>/results.json
    artifacts/search/<run_id>/<viewport>/<CASE_ID>-failure.png
...
from tests.search_cases import SearchCaseRunner
...
ARTIFACT_ROOT = PROJECT_ROOT / "artifacts" / "search"


def run_viewport(viewport: str, artifact_dir: Path) -> Tuple[List, SearchCaseRunner, dict]:
    ...
    try:
        ...
        runner = SearchCaseRunner(runtime, site, viewport, artifact_dir=artifact_dir)
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Mondressy Search Cases")
    ...
    print()
    print("Results:")
    print(f"artifacts/search/{run_id}/results.json")
