"""Home + Navigation Cases 入口。

用法：
    python scripts/run_navigation_cases.py                   # both
    python scripts/run_navigation_cases.py --viewport desktop
    python scripts/run_navigation_cases.py --viewport mobile

产物：
    artifacts/navigation/<run_id>/results.json
    artifacts/navigation/<run_id>/<viewport>/<CASE_ID>-failure.png
...
from tests.navigation_cases import NavigationCaseRunner
...
ARTIFACT_ROOT = PROJECT_ROOT / "artifacts" / "navigation"


def run_viewport(viewport: str, artifact_dir: Path) -> Tuple[List, NavigationCaseRunner, dict]:
    ...
    try:
        ...
        runner = NavigationCaseRunner(runtime, site, viewport, artifact_dir=artifact_dir)
...
def print_viewport(viewport: str, results: List) -> None:
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Mondressy Home + Navigation Cases")
    ...
    print("=== Mondressy Home + Navigation Cases ===")
    print()
    ...
    for vp in viewports:
        ...
        vp_results.append(
            ViewportResult(
                ...
                pre_clean={"status": "PASS", "residual_items_before": 0},
                cleanup={"status": "PASS", "residual_items_after": 0},
                cases=results,
            )
        )
        print_viewport(vp, results)
    ...
    print()
    print("Results:")
    print(f"artifacts/navigation/{run_id}/results.json")
