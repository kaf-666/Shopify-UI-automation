"""Direct PDP Purchase Cases 入口。

用法：
    python scripts/run_direct_pdp_cases.py                   # both
    python scripts/run_direct_pdp_cases.py --viewport desktop
    python scripts/run_direct_pdp_cases.py --viewport mobile

产物：
    artifacts/direct-pdp/<run_id>/results.json
    artifacts/direct-pdp/<run_id>/<viewport>/<CASE_ID>-failure.png
...
from tests.direct_pdp_cases import DirectPdpCaseRunner
...
ARTIFACT_ROOT = PROJECT_ROOT / "artifacts" / "direct-pdp"


def run_viewport(viewport: str, artifact_dir: Path) -> Tuple[List, DirectPdpCaseRunner, dict]:
    ...
    try:
        ...
        runner = DirectPdpCaseRunner(runtime, site, viewport, artifact_dir=artifact_dir)
...
def print_viewport(viewport: str, results: List, runner: Optional[DirectPdpCaseRunner] = None) -> None:
    print(f"[{viewport.title()} / {'Chromium' if viewport == 'desktop' else 'WebKit + iPhone 14'}]")
    counts = {"PASS": 0, "FAIL": 0, "BLOCKED": 0}
    for r in results:
        counts[r.status] += 1
        print(f"{r.case_id:<16} {r.status:<8} {r.name}")
        if r.detail:
            print(f"{'':<16} {'':<8} {r.detail}")
    print(f"{'':<16} PASS={counts['PASS']} FAIL={counts['FAIL']} BLOCKED={counts['BLOCKED']} TOTAL={len(results)}")
    if runner is not None:
        print(f"{'':<16} Pre-clean: {runner.pre_clean_status} | Cleanup: {runner.cleanup_status}")
    print()
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Mondressy Direct PDP Purchase Cases")
    ...
    print("=== Mondressy Direct PDP Purchase Cases ===")
    print()
        ...
        vp_results.append(
            ViewportResult(
                ...
                cleanup={
                    ...
                    "detail": runner.cleanup_error or "",
                },
                cases=results,
            )
        )
    ...
    print()
    print("Results:")
    print(f"artifacts/direct-pdp/{run_id}/results.json")
